...
export const BottomNav = () => {
  ...
  const profileLink = isMounted && user ? `/users/${user.username}` : '/login';
      ...
      <div className='bg-neutral-950/90 backdrop-blur-md border border-neutral-800 rounded-full px-6 py-3 flex items-center justify-between shadow-2xl'>
        ...
        <Link
          ...
          className={`flex flex-col items-center gap-1 transition-colors ${
            pathname === '/'
              ? 'text-primary-200'
              : 'text-neutral-500 hover:text-base-white'
          }`}
        >
          <Home
            className={`w-5 h-5 ${pathname === '/' ? 'fill-primary-200' : ''}`}
          />
        ...
        >
          <User
            className={`w-5 h-5 ${isProfileActive ? 'fill-primary-200' : ''}`}
          />
